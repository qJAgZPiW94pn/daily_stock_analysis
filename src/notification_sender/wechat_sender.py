...
职责：
1. 通过企业微信 Webhook 发送文本訊息
2. 通过企业微信 Webhook 发送图片訊息
"""
import logging
import base64
import hashlib
...
class WechatSender:
    ...
    def send_to_wechat(self, content: str, *, timeout_seconds: Optional[float] = None) -> bool:
        ...
        if not self._wechat_url:
            logger.warning("企业微信 Webhook 未配置，略過推送")
            return False
        
        # 根据訊息类型动态限制上限，避免 text 类型超过企业微信 2048 字节限制
    ...
    def _send_wechat_message(self, content: str, *, timeout_seconds: Optional[float] = None) -> bool:
        ...
        if response.status_code == 200:
            result = response.json()
            if result.get('errcode') == 0:
                logger.info("企业微信訊息发送成功")
                return True
            else:
                logger.error(f"企业微信傳回錯誤: {result}")
                return False
        else:
            logger.error(f"企业微信請求失败: {response.status_code}")
            return False
