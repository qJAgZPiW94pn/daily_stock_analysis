    ...
    def send_to_ntfy(
        ...
        *,
        ...
        if not self._is_ntfy_configured():
            logger.warning("ntfy URL 未配置，略過推送")
            return False
        ...
        try:
            ...
            if 200 <= response.status_code < 300:
                logger.info("ntfy 訊息发送成功")
                return True

            logger.error("ntfy 請求失败: HTTP %s", response.status_code)
            logger.debug("ntfy 回應内容: %s", response.text)
            return False
        except requests.exceptions.Timeout:
            logger.error("发送 ntfy 訊息失败: 請求逾時")
            return False
        except requests.exceptions.RequestException as exc:
            logger.error("发送 ntfy 訊息失败: 網路請求例外")
            logger.debug("ntfy 請求例外类型: %s", type(exc).__name__)
            return False
        except Exception as exc:
            logger.error("发送 ntfy 訊息失败: 未知例外")
            logger.debug("ntfy 未知例外类型: %s", type(exc).__name__)
            return False
