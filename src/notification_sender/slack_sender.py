...
# Slack Block Kit 中单个 section block 的 text 欄位上限为 3000 字符
_BLOCK_TEXT_LIMIT = 3000
# Slack chat.postMessage / Webhook 的 text 欄位上限约 40000 字符，保守取 39000
_TEXT_LIMIT = 39000
...
class SlackSender:
    ...
    def send_to_slack(self, content: str, *, timeout_seconds: Optional[float] = None) -> bool:
        ...
        logger.warning("Slack 配置不完整，略過推送")
        return False
    ...
    def _send_slack_webhook(self, content: str, *, timeout_seconds: Optional[float] = None) -> bool:
        """
        使用 Incoming Webhook 发送訊息到 Slack
        ...
        try:
            ...
            if response.status_code == 200 and response.text == "ok":
                logger.info("Slack Webhook 訊息发送成功")
                return True
    ...
    def _send_slack_bot(self, content: str, *, timeout_seconds: Optional[float] = None) -> bool:
        """
        使用 Bot API (chat.postMessage) 发送訊息到 Slack
    ...
    def _send_slack_image(self, image_bytes: bytes, fallback_content: str = "") -> bool:
        ...
        (Slack 新版文件上傳 API)；Webhook 模式下回退为文本。
        ...
        if self._use_bot:
            ...
            try:
                ...
                # Step 2: 上傳文件内容（raw body，不能用 multipart）
                resp2 = requests.post(
                    upload_url,
                    data=image_bytes,
                ...
                if resp2.status_code != 200:
                    logger.error("Slack 文件上傳失败: HTTP %s", resp2.status_code)
        ...
        if fallback_content:
            logger.info("Slack 图片不支援或失败，回退为文本发送")
