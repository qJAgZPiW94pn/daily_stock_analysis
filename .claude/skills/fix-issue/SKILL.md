...
基于 issue 分析结果实现修复，并按倉庫规则补齐验证、風險与回滚说明。
...
优先先完成 `/analyze-issue <issue_number>`，确保議題成立且边界清晰。
...
- 默认基于当前工作树做最小相关改动
- 不要默认执行 `git pull`
- 不要默认切换分支或改写使用者当前工作狀態
- 如果使用者明确要求建分支，再执行最小必要的分支操作
...
- 如果修复涉及使用者可见行为、配置语义、CLI/API、部署、通知、报告结构，要同步更新相关文档、`docs/CHANGELOG.md`、`.env.example`
...
- 更细的模块行为、页面交互、专题配置、排障说明、欄位契约、实现语义和边界条件，优先更新对应 `docs/*.md`
