...
def view_database():
    ...
    try:
        ...
        print_section(f"已存储股票數據 (共 {len(stocks)} 只)")
        if stocks:
            print(f"  {'代碼':<10} {'记录数':<8} {'起始日期':<12} {'最新日期':<12} {'數據源'}")
            print("  " + "-" * 60)
            for row in stocks:
        ...
        if today_data:
            for row in today_data:
                ...
                print(f"    漲跌幅: {pct_chg:.2f}%  成交量: {volume/10000:.2f}万股")
        ...
        if recent:
            print(f"  {'代碼':<10} {'日期':<12} {'收盤':<10} {'漲跌%':<8} {'成交量':<15} {'来源'}")
            print("  " + "-" * 70)
            for row in recent:
