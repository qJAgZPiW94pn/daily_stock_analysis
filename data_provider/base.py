...
- BaseFetcher: 抽象基类，定义统一介面
- DataFetcherManager: 策略管理器，實現自动切換
...
def _is_hk_market(code: str) -> bool:
    ...
    支援 `HK00700` 及纯 5 位数字形式（A 股 ETF/股票常见为 6 位）。
    """
    normalized = (code or "").strip().upper()
...
class BaseFetcher(ABC):
    ...
    子类實現：
    - _fetch_raw_data(): 从具体數據源獲取原始數據
    - _normalize_data(): 将原始數據轉換为標準格式
    """
    
    name: str = "BaseFetcher"
    priority: int = 99  # 優先级数字越小越優先
    ...
    def get_daily_data(
        ...
        Args:
            stock_code: 股票代碼
            start_date: 开始日期（可選）
            end_date: 结束日期（可選，預設今天）
            days: 獲取天数（当 start_date 未指定时使用）
            
        Returns:
            標準化的 DataFrame，包含技术指標
        """
        # 计算日期範圍
        if end_date is None:
        ...
        if start_date is None:
            # 預設獲取最近 30 个交易日（按日历日估算，多取一些）
            from datetime import timedelta
            start_dt = datetime.strptime(end_date, '%Y-%m-%d') - timedelta(days=days * 2)
            start_date = start_dt.strftime('%Y-%m-%d')

        request_start = time.time()
        logger.info(f"[{self.name}] 开始獲取 {stock_code} 日线數據: 範圍={start_date} ~ {end_date}")
        ...
        try:
            ...
            logger.error(
                f"[{self.name}] {stock_code} 獲取失败: 範圍={start_date} ~ {end_date}, "
    ...
    def _calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 量比：当日成交量 / 5日平均成交量
        # 注意：此处的 volume_ratio 是“日线成交量 / 前5日均量(shift 1)”的相對倍数，
...
class DataFetcherManager:
    ...
    def _call_availability_probe(fetcher: BaseFetcher, probe_name: str, capability: str) -> Optional[bool]:
        ...
        try:
            ...
            logger.debug(
                "[數據源可用性] %s.%s 檢查失败(capability=%s): %s",
                fetcher.name,
                probe_name,
        ...
        with self._tickflow_lock:
            ...
            if not api_key:
                if current_fetcher is not None and hasattr(current_fetcher, "close"):
                    try:
                        ...
                        logger.debug("[TickFlowFetcher] 關閉旧实例失败: %s", exc)
                self._tickflow_fetcher = None
            ...
            if current_fetcher is not None and hasattr(current_fetcher, "close"):
                try:
                    ...
                    logger.debug("[TickFlowFetcher] 切換实例时關閉失败: %s", exc)
    ...
    def close(self) -> None:
        ...
        if current_fetcher is not None and hasattr(current_fetcher, "close"):
            try:
                ...
                logger.debug("[TickFlowFetcher] 關閉管理器资源失败: %s", exc)
    ...
    def _get_fundamental_cache_key(self, stock_code: str, budget_seconds: Optional[float] = None) -> str:
        """生成基本面快取 key（包含预算分桶以避免低预算结果污染高预算請求）。"""
        ...
        if isinstance(raw_data, pd.DataFrame):
            ...
            name_col = next(
                (
                    ...
                    if str(col) in {"板塊名称", "板塊", "所属板塊", "板塊名", "name", "industry"}
                ),
                None,
            )
            code_col = next(
    ...
    def _init_default_fetchers(self) -> None:
        ...
        優先级动态調整邏輯：
        - 如果配置了 TUSHARE_TOKEN：实例化 TushareFetcher，并按其内部邏輯提升優先级
        - 如果配置了 Longbridge 凭据：实例化 LongbridgeFetcher 作为美股/港股兜底
        - 未配置的可選數據源不实例化，避免在批量拉取时反复探测无效源
        - 預設優先级：
          0. EfinanceFetcher (Priority 0) - 最高優先级
        ...
        config = get_config()
        # 建立所有數據源实例（優先级在各 Fetcher 的 __init__ 中確定）
        efinance = EfinanceFetcher()
        akshare = AkshareFetcher()
        pytdx = PytdxFetcher()      # 通达信數據源（可配 PYTDX_HOST/PYTDX_PORT）
        ...
        if tushare_token:
            optional_fetchers.append(TushareFetcher())  # 会根据 Token 配置自动調整優先级
        else:
            logger.debug("[數據源初始化] 略過未配置的 TushareFetcher")
        ...
        if has_longbridge_creds:
            ...
            logger.debug("[數據源初始化] 略過未配置的 LongbridgeFetcher")
        ...
        if finnhub_api_key:
            ...
            logger.debug("[數據源初始化] 略過未配置的 FinnhubFetcher")
        ...
        if alphavantage_api_key:
            ...
            logger.debug("[數據源初始化] 略過未配置的 AlphaVantageFetcher")
        ...
        with self._fetchers_lock:
            ...
            # 按優先级排序（Tushare 如果配置了 Token 且初始化成功，優先级为 0）
            self._fetchers.sort(key=lambda f: f.priority)
            self._refresh_fetcher_indexes_locked()

        # 构建優先级说明
        priority_info = ", ".join([f"{f.name}(P{f.priority})" for f in self._get_fetchers_snapshot()])
        logger.info(f"已初始化 {len(self._fetchers)} 个數據源（按優先级）: {priority_info}")
    ...
    def get_daily_data(
        ...
        Returns:
            Tuple[DataFrame, str]: (數據, 成功的數據源名称)
            
        Raises:
            DataFetchError: 所有數據源都失败时拋出
        """
        ...
        if total_fetchers == 0:
            ...
            error_summary = f"{market_label} {stock_code} 獲取失败:\n暂无可用數據源"
            logger.error(f"[數據源终止] {stock_code} 獲取失败: {error_summary}")
            raise DataFetchError(error_summary)

        # 美股（含美股指數）使用专用路由；港股走下方通用數據源循环
        ...
        if is_us:
            ...
            if is_us_index:
                # 指數始终 YFinance 首选（Longbridge 不提供指數K线）
            ...
            for src_name in source_order:
                for attempt, fetcher in enumerate(fetchers, start=1):
                    ...
                    try:
                        ...
                        logger.warning(
                            f"[數據源失败 {attempt}/{total_fetchers}] [{fetcher.name}] {stock_code}: "
            ...
            error_summary = f"{market_label} {stock_code} 獲取失败:\n" + "\n".join(errors)
            elapsed = time.time() - request_start
            logger.error(f"[數據源终止] {stock_code} 獲取失败: elapsed={elapsed:.2f}s\n{error_summary}")
            raise DataFetchError(error_summary)
        ...
        # 所有數據源都失败
        error_summary = f"所有數據源獲取 {stock_code} 失败:\n" + "\n".join(errors)
        elapsed = time.time() - request_start
        logger.error(f"[數據源终止] {stock_code} 獲取失败: elapsed={elapsed:.2f}s\n{error_summary}")
        raise DataFetchError(error_summary)
    ...
    def prefetch_realtime_quotes(self, stock_codes: List[str]) -> int:
        ...
        2. 如果不包含，略過预取（新浪/腾讯是单股票查詢，無需预取）
        3. 如果自选股数量 >= 5 且使用全量數據源，则预取填充快取
        
        这样做的好处：
        - 使用新浪/腾讯时：每只股票独立查詢，无全量拉取議題
        - 使用 efinance/东财时：预取一次，后续快取命中
        ...
        if not getattr(config, "prefetch_realtime_quotes", True):
            logger.debug("[预取] PREFETCH_REALTIME_QUOTES=false，略過批量预取")
            return 0

        # 如果实时行情被禁用，略過预取
        if not config.enable_realtime_quote:
            logger.debug("[预取] 实时行情功能已禁用，略過预取")
            return 0
        
        # 檢查優先级中是否包含全量拉取數據源
        # 注意：新增全量介面（如 tushare_realtime）时需同步更新此列表
        # 全量介面特征：一次 API 呼叫拉取全市场 5000+ 股票數據
        priority = config.realtime_source_priority.lower()
        bulk_sources = ['efinance', 'akshare_em', 'tushare']  # 全量介面列表
        
        # 如果優先级中前两个都不是全量數據源，略過预取
        # 因为新浪/腾讯是单股票查詢，不需要预取
        ...
        # 如果没有全量數據源，或者全量數據源排在第 3 位之后，略過预取
        if first_bulk_source_index is None or first_bulk_source_index >= 2:
            logger.info(f"[预取] 当前優先级使用轻量级數據源(sina/tencent)，無需预取")
            return 0
        
        # 如果股票数量少于 5 个，不進行批量预取（逐个查詢更高效）
        if len(stock_codes) < 5:
            logger.info(f"[预取] 股票数量 {len(stock_codes)} < 5，略過批量预取")
            return 0
        
        logger.info(f"[预取] 开始批量预取实时行情，共 {len(stock_codes)} 只股票...")
        
        # 嘗試通过 efinance 或 akshare 预取
        # 只需要呼叫一次 get_realtime_quote，快取機制会自动拉取全市场數據
        try:
            # 用第一只股票触发全量拉取
            ...
            if quote:
                logger.info(f"[预取] 批量预取完成，快取已填充")
                return len(stock_codes)
            else:
                logger.warning(f"[预取] 批量预取失败，将使用逐个查詢模式")
                return 0
                
        except Exception as e:
            logger.error(f"[预取] 批量预取例外: {e}")
            return 0
    ...
    def get_realtime_quote(self, stock_code: str, *, log_final_failure: bool = True):
        ...
        Returns:
            UnifiedRealtimeQuote 对象，所有數據源都失败则傳回 None
        """
        ...
        # 如果实时行情功能被禁用，直接傳回 None
        if not config.enable_realtime_quote:
            logger.debug(f"[实时行情] 功能已禁用，略過 {stock_code}")
            return None
        ...
        #   美股指數:   始终 YFinance 首选（Longbridge 不提供指數行情）
        # ----------------------------------------------------------
        is_us_index = is_us_index_code(stock_code)
        ...
        if is_us or is_hk:
            ...
            if primary_quote is not None:
                logger.info(f"[实时行情] {market_label} {stock_code} 成功獲取 (来源: {primary_src})")
            primary_quote = self._supplement_quote(
                stock_code, primary_quote, secondary_src, **secondary_kw,
            )
            # 美股個股（非指數）嘗試从 Finnhub/AlphaVantage 补充缺失欄位
            ...
            if log_final_failure:
                logger.info(f"[实时行情] {market_label} {stock_code} 无可用數據源")
            return None
            ...
            try:
                ...
                if quote is not None and quote.has_basic_data():
                    if primary_quote is None:
                        ...
                        primary_quote = quote
                        logger.info(f"[实时行情] {stock_code} 成功獲取 (来源: {source})")
                        ...
                        logger.debug(f"[实时行情] {stock_code} 部分欄位缺失，嘗試从后续數據源补充")
                        ...
                        if supplement_attempts > 1:
                            logger.debug(f"[实时行情] {stock_code} 补充嘗試已达上限，停止繼續")
                            break
                        merged = self._merge_quote_fields(primary_quote, quote)
                        if merged:
                            logger.info(f"[实时行情] {stock_code} 从 {source} 补充了缺失欄位: {merged}")
                ...
                logger.info(f"[实时行情] {stock_code} {error_msg}，繼續嘗試下一个數據源")
                errors.append(error_msg)
                continue
        ...
        # 所有數據源都失败，傳回 None（降級兜底）
        if log_final_failure:
            if errors:
                logger.info(f"[实时行情] {stock_code} 所有數據源均失败: {'; '.join(errors)}")
            else:
                logger.info(f"[实时行情] {stock_code} 无可用數據源")
    ...
    def _try_fetcher_quote(self, stock_code: str, fetcher_name: str, **kw):
        ...
        try:
            ...
            logger.debug(f"[实时行情] {stock_code} {fetcher_name} 獲取失败: {e}")
        return None
    ...
    def _supplement_quote(self, stock_code: str, primary_quote, fetcher_name: str, **kw):
        ...
        if q is not None:
            logger.info(f"[实时行情] {stock_code} 从 {fetcher_name} 獲取成功 (独立數據源)")
        return q
    ...
    def get_chip_distribution(self, stock_code: str):
        ...
        1. 檢查配置开关
        2. 檢查熔斷器狀態
        3. 依次嘗試多个數據源：數據源優先级与獲取daily的數據優先级一致
        4. 所有數據源失败则傳回 None（降級兜底）

        Args:
            stock_code: 股票代碼

        Returns:
            ChipDistribution 对象，失败则傳回 None
        """
        # Normalize code (strip SH/SZ prefix etc.)
        stock_code = normalize_stock_code(stock_code)
        ...
        # 如果筹码分布功能被禁用，直接傳回 None
        if not config.enable_chip_distribution:
            logger.debug(f"[筹码分布] 功能已禁用，略過 {stock_code}")
            return None
        ...
        for fetcher in self._get_fetchers_snapshot():
            ...
            # 动态生成熔斷器的 key，例如 "TushareFetcher" -> "tushare_chip"
            source_key = f"{fetcher_name.replace('Fetcher', '').lower()}_chip"

            # 檢查熔斷器狀態
            if not circuit_breaker.is_available(source_key):
                logger.debug(f"[熔斷] {fetcher_name} 筹码介面处于熔斷狀態，嘗試下一个")
                continue
            ...
            try:
                ...
                if chip is not None:
                    ...
                    logger.info(f"[筹码分布] {stock_code} 成功獲取 (来源: {fetcher_name})")
                    return chip
                else:
                    # 空结果：释放 HALF_OPEN 探测名额，避免卡死
                    circuit_breaker.record_inconclusive(source_key)
            except Exception as e:
                logger.warning(f"[筹码分布] {fetcher_name} 獲取 {stock_code} 失败: {e}")
                circuit_breaker.record_failure(source_key, str(e))
                continue

        logger.warning(f"[筹码分布] {stock_code} 所有數據源均失败")
        return None
    ...
    def get_stock_name(self, stock_code: str, allow_realtime: bool = True) -> Optional[str]:
        ...
        嘗試从多个數據源獲取股票名称：
        1. 先从記憶體快取中獲取（如果有）
        2. 再嘗試本機维护映射与 stocks.index.json 索引
        3. 然后按需查詢实时行情
        4. 依次嘗試各个數據源的 get_stock_name 方法
        ...
        if allow_realtime:
            ...
            if quote and hasattr(quote, 'name') and is_meaningful_stock_name(getattr(quote, 'name', ''), stock_code):
                ...
                logger.info(f"[股票名称] 从实时行情獲取: {stock_code} -> {name}")
                return name
        ...
        for fetcher in self._get_fetchers_snapshot():
            ...
            try:
                ...
                if is_meaningful_stock_name(name, stock_code):
                    self._cache_stock_name(stock_code, name)
                    logger.info(f"[股票名称] 从 {fetcher.name} 獲取: {stock_code} -> {name}")
                    return name
            except Exception as e:
                logger.debug(f"[股票名称] {fetcher.name} 獲取失败: {e}")
                continue

        # 4. 所有數據源都失败
        logger.warning(f"[股票名称] 所有數據源都無法獲取 {stock_code} 的名称")
        return ""
    ...
    def get_belong_boards(self, stock_code: str) -> List[Dict[str, Any]]:
        ...
        for fetcher in self._fetchers:
            ...
            try:
                ...
                if boards:
                    logger.info(f"[{fetcher.name}] 獲取所属板塊成功: {stock_code}, count={len(boards)}")
                    return boards
            except Exception as e:
                logger.debug(f"[{fetcher.name}] 獲取所属板塊失败: {e}")
                continue
        return []
    ...
    def batch_get_stock_names(self, stock_codes: List[str]) -> Dict[str, str]:
        ...
        for fetcher in self._get_fetchers_snapshot():
            ...
            try:
                ...
                if stock_list is not None and not stock_list.empty:
                    ...
                    logger.info(f"[股票名称] 从 {fetcher.name} 批量獲取完成，剩余 {len(missing_codes)} 个待查")
            except Exception as e:
                logger.debug(f"[股票名称] {fetcher.name} 批量獲取失败: {e}")
                continue
        ...
        logger.info(f"[股票名称] 批量獲取完成，成功 {len(result)}/{len(stock_codes)}")
        return result
    ...
    def get_main_indices(self, region: str = "cn") -> List[Dict[str, Any]]:
        ...
        if region == "cn":
            ...
            if tickflow_fetcher is not None:
                try:
                    ...
                    logger.warning(f"[TickFlowFetcher] 獲取指數行情失败: {e}")
        ...
        for fetcher in self._fetchers:
            try:
                ...
                logger.warning(f"[{fetcher.name}] 獲取指數行情失败: {e}")
                continue
        return []
    ...
    def get_market_stats(self) -> Dict[str, Any]:
        ...
        if tickflow_fetcher is not None:
            try:
                ...
                if data:
                    logger.info("[TickFlowFetcher] 獲取市场統計成功")
                    return data
            except Exception as e:
                logger.warning(f"[TickFlowFetcher] 獲取市场統計失败: {e}")

        for fetcher in self._fetchers:
            try:
                data = fetcher.get_market_stats()
                if data:
                    logger.info(f"[{fetcher.name}] 獲取市场統計成功")
                    return data
            except Exception as e:
                logger.warning(f"[{fetcher.name}] 獲取市场統計失败: {e}")
                continue
        return {}
    ...
    def _get_sector_rankings_with_meta(
            ...
            for fetcher in self._fetchers:
                ...
                try:
                    ...
                    last_error = f"{fetcher.name}傳回空结果"
                    source_chain.append(
                        {
                            "provider": fetcher.name,
                            "result": "empty",
                    ...
                    logger.warning(f"[{fetcher.name}] 獲取板塊排行失败: {error_reason}")
    ...
    def get_sector_rankings(self, n: int = 5) -> Tuple[List[Dict], List[Dict]]:
        ...
        if top or bottom:
            return top, bottom
        logger.warning(f"[板塊排行] 所有數據源均失败，最終錯誤: {last_error}")
        return [], []
    ...
    def get_concept_rankings(self, n: int = 5) -> Tuple[List[Dict], List[Dict]]:
        ...
        for fetcher in self._fetchers:
            try:
                ...
                error_type, error_reason = summarize_exception(e)
                last_error = f"{fetcher.name} ({error_type}) {error_reason}"
                logger.warning(f"[{fetcher.name}] 獲取概念排行失败: {error_reason}")
        if last_error:
            logger.warning(f"[概念排行] 所有數據源均失败，最終錯誤: {last_error}")
        return [], []
    ...
    def get_hot_stocks(self, n: int = 10) -> List[Dict[str, Any]]:
        ...
        for fetcher in self._fetchers:
            try:
                ...
                if data:
                    logger.info(f"[{fetcher.name}] 獲取人气股成功")
                    return data[:n]
                last_error = f"{fetcher.name}傳回空结果"
            except Exception as e:
                error_type, error_reason = summarize_exception(e)
                last_error = f"{fetcher.name} ({error_type}) {error_reason}"
                logger.warning(f"[{fetcher.name}] 獲取人气股失败: {error_reason}")
        if last_error:
            logger.warning(f"[人气股] 所有數據源均失败，最終錯誤: {last_error}")
        return []
    ...
    def get_limit_up_pool(
        ...
        for fetcher in self._fetchers:
            try:
                ...
                error_type, error_reason = summarize_exception(e)
                last_error = f"{fetcher.name} ({error_type}) {error_reason}"
                logger.warning(f"[{fetcher.name}] 獲取漲停池失败: {error_reason}")
        if last_error:
            logger.warning(f"[漲停池] 所有數據源均失败，最終錯誤: {last_error}")
        return []
