...
class TickFlowFetcher(BaseFetcher):
    ...
    def close(self) -> None:
        ...
        if client is not None:
            try:
                ...
                logger.debug("[TickFlowFetcher] 關閉客户端失败: %s", exc)
    ...
    def get_main_indices(self, region: str = "cn") -> Optional[List[Dict[str, Any]]]:
        ...
        if not quotes:
            logger.warning("[TickFlowFetcher] 指數行情为空")
            return None
    ...
    def get_market_stats(self) -> Optional[Dict[str, Any]]:
        ...
        for quote in quotes:
            ...
            if not name:
                logger.debug("[TickFlowFetcher] 缺少股票名称，按非 ST 處理: %s", symbol)
