...
如果你的机器人开启了额外安全项，项目侧也必须同步配置，否则請求会被飞书拒绝。
...
项目会自动在每条飞书訊息前补上这个关键词，你不需要手工改报告模板。
...
### 2. 飞书机器人开启了关键词，但本機没配 `FEISHU_WEBHOOK_KEYWORD`
...
### 3. 飞书机器人开启了签名校验，但本機没配 `FEISHU_WEBHOOK_SECRET`
...
5. 最后再检查机器人是否在群里、是否有權限、是否命中 IP 白名单

## 附：应用 / Stream Bot 原流程截图参考

如果你不是单纯做群 Webhook 推送，而是要繼續配置飞书应用、长連線机器人或云文档，可以参考下面这组原截图。
