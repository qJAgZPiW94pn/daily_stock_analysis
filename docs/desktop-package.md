...
当前 Windows 安裝包使用 NSIS 向导式安裝流程，仅支援当前使用者安裝且已禁用管理员提权，安裝时可手动选择目标目錄（例如非 C 盘）。安裝器通过 NSIS `.onVerifyInstDir` 回调在安裝器层面阻止选择 `Program Files`、`Windows` 等系統保护目錄——选择这些路徑时"下一步"按钮会被自动禁用。安裝完成后，桌面端仍会按现有邏輯在安裝目錄旁生成/讀取 `.env`、`data/stock_analysis.db`（含 `data/stock_analysis.db-wal` / `data/stock_analysis.db-shm`）和 `logs/desktop.log`。推荐使用預設的 per-user 安裝目錄。如果不想安裝，仍可繼續分发 `win-unpacked` 免安裝包。
...
桌面端自动更新鏈路依賴 Windows NSIS 安裝产物、`latest.yml` 与 `*.blockmap` 元數據。当前桌面 CI 不覆盖 `desktop-release` 打包产物可發佈鏈路，提交前建议补充如下本機驗證：

说明：该清单专注于 Windows NSIS 安裝版与 `electron-updater` 發佈元數據。当前 Linux 環境無法直接产出 Windows 安裝包和 updater 元數據（`latest.yml` / `*.blockmap`），此类鏈路需在 Windows 發佈執行器或 Windows 本机環境复核。

若在非 Windows 環境無法完成上述驗證，请在 PR 验收说明中明確补齐 Windows 發佈鏈路复核人、复核时间窗及 `desktop-release` 产物檢查结果（release/tag 与 `daily-stock-analysis-windows-installer-<tag>.exe`、`latest.yml`、`*.blockmap` 版本一致性与可下載性）。
...
2. 回到桌面端，补齐依賴、執行 preload 单测、再執行 Electron 打包
...
4. 強制对齐版本与發佈附件（可在 Windows 環境或能产出 NSIS 产物的執行器上复核）
...
echo \"- latest.yml\"
echo \"- *.blockmap\"
echo \"并確保 latest.yml 中 version 与 tag 的语义化版本一致，path/url 与安裝包附件名一致\"
```

5a. 建议在 PR 描述里记录的“可复核輸出”（Windows）：
...
Windows 發佈鏈路复核清单（在 PR 后由發佈团队/维护者執行）：
...
- `latest.yml` 中 `version` 与 Release tag 语义一致（去掉 `v` 前綴后比对），且 `path` / `files.url` 与安裝包附件名一致；
- 如缺少上述文件或 `release-tag` 不匹配，需标注阻断并补齐 `desktop-release` 打包流程。

5. Windows/NSIS 产物与發佈附件一致性请在 Windows 環境手动驗證（可人工触发發佈流程），并在升級后核对執行时文件留存：
   ...
   3. 如不一致，可在应用退出后檢查使用者數據目錄中的 `.dsa-desktop-update-backup` 是否清理完整，并结合最新日誌串联排查。

Windows 平台建议使用 PowerShell 執行：
...
说明：应用已在 Windows NSIS 安裝版的“重啟安裝”前備份安裝目錄旁上述執行时文件并嘗試恢復，目的是降低更新过程中文件丢失風險；若恢復失败，桌面端会顯示更新安裝錯誤并保留手动下載路徑供回退處理。
...
Windows 安裝包模式下，安裝器仅支援当前使用者安裝且已禁用管理员提权，使用者可在安裝向导中选择安裝目錄；安裝器会在安裝器层面阻止选择 `Program Files`、`Windows` 等系統保护目錄（选择时"下一步"按钮自动禁用），安裝完成后，应用会在安裝目錄旁生成/讀取 `.env`、`data/stock_analysis.db`（含 `data/stock_analysis.db-wal` / `data/stock_analysis.db-shm`）和 `logs/desktop.log`。请保留預設的 per-user 安裝位置或选择其他使用者可写目錄。
...
- macOS 打包态下，`exe` 實際位于 `.app` 包内部，因此 `.env`、`data/`、`logs/` 也会跟着落在应用包内容器里；替換新的 DMG / `.app` 时，旧配置会随旧应用包一起被覆盖
- 使用者需要编辑 `.env` 配置以下内容：
...
- `匯入 .env` 会讀取備份文件中的键值并合併到当前配置中，匯入后会立即触发配置重載
- 匯入是“键级覆盖”而不是整文件替換：備份文件中出现的键会覆盖当前值，未出现的键保持不变
- 如果当前页面还有未保存草稿，匯入前会先提示确认，避免把本機草稿和已保存配置混在一起
- Web 端預設 `ADMIN_AUTH_ENABLED=false` 时，设置页会展示按钮为禁用态并提示先启用管理员鉴权；桌面端不受该配置影响，仍可直接使用配置備份/恢復能力。

> 建议：macOS 使用者在升級 DMG 前先執行一次 `匯出 .env`，这样即使旧 `.app` 被整体替換，也能在新版本里直接恢復配置

### 设置页版本資訊

- `系統设置 -> 版本資訊` 中的“桌面端版本”由 Electron 主程式的 `app.getVersion()` 提供，并通过 preload bridge 暴露给前端
- 开发态 `npm run dev` 与打包态 `npm run build` / 安裝包都会复用同一条版本注入鏈路，不再在 `preload.js` 里维护独立硬編碼版本号
- `README.md` 繼續保留安裝和執行入口说明；这类桌面端執行时细节统一落在本专题文档维护，避免入门文档膨胀

### 桌面端更新提醒

- 应用在主界面加载完成后会后台檢查 GitHub Releases 的最新正式版，并与当前 `app.getVersion()` 做语义化版本比较
- Windows NSIS 安裝版会通过内置 GitHub 更新源自动下載新版本；下載完成后弹出一次性提醒，使用者确认后重啟并安裝
- `系統设置 -> 版本資訊` 中的“桌面端更新”区域可手动檢查更新；若更新已下載，会展示“重啟安裝”操作
- Windows 免安裝包、开发态和 macOS DMG 仍保持“提醒 + 跳转下載页”的相容路徑，不会因为網路失败而阻断桌面端啟動
- 版本檢查失败、GitHub API 逾時、更新元數據缺失或下載安裝例外时，会记录到 `logs/desktop.log`，设置页手动檢查时会展示錯誤狀態

## 常见議題

### 啟動后一直顯示 "Preparing backend..."

1. 檢查 `logs/desktop.log` 查看錯誤資訊
2. 确认 `.env` 文件存在且配置正确
3. 确认端口 8000-8100 未被占用

### 后端啟動报 ModuleNotFoundError

PyInstaller 打包时缺少模組，需要在 `scripts/build-backend.ps1` 中增加 `--hidden-import`。
...
这是当前桌面端便携模式的已知行为：`.env` 放在打包后的应用目錄旁，而 macOS 中这个目錄通常位于 `.app` 包内部。升級或替換新的 DMG / `.app` 后，旧 `.env` 不会自动迁移，所以看起来像“配置丢了”。

處理方式：

1. 升級前在桌面端设置页執行一次 `匯出 .env`
