...
本文档记录 `src/services/image_stock_extractor.py` 中 `EXTRACT_PROMPT` 的完整内容，便于 PR 审查时評估指令效果。

**当修改 EXTRACT_PROMPT 时**：请同步更新此文件，并在 PR 描述中展示完整变更（before/after），以便审查者評估针对 code+name+confidence 提取的最佳化程度。
...
```
请分析这张股票市场截图或图片，提取其中所有可见的股票代碼及名称。
...
- confidence: 必填，识别置信度，high=確定、medium=较確定、low=不確定

示例（图中同时有名称和代碼时）：
- 個股：600519 贵州茅台、300750 宁德时代
