...
本指南详细介绍如何在 Zeabur 上部署 A股自选股智能分析系統，包括 WebUI 和 Discord 机器人功能。
...
- 相关 API 密钥（如 Gemini API Key、搜索服務 API Key 等）
...
1. 等待镜像构建完成
2. 点击「啟動服務」
3. 服務啟動后，你可以在「访问」標籤页获取访问地址

### 2.4 前端构建与静态资源

FastAPI 会自动托管 `static/` 目錄下的前端资源。前端打包输出位置由
`apps/dsa-web/vite.config.ts` 决定，默认输出到项目根目錄 `static/`。
...
| 模式 | 啟動命令 | 描述 |
|------|----------|------|
| 定时工作模式（默认） | `python main.py --schedule` | 按计划执行股票分析 |
| FastAPI 模式 | `python main.py --serve` | 啟動 FastAPI 并执行分析 |
| 仅 FastAPI 模式 | `python main.py --serve-only` | 仅啟動 FastAPI，不执行分析 |
| 仅大盤复盘 | `python main.py --market-review` | 仅执行大盤复盘分析 |
...
机器人功能默认通过配置启用，無需特殊啟動命令。确保你的配置文件中包含机器人相关配置，或通过环境變數设置。
...
> 旧版 `WEBUI_HOST`/`WEBUI_PORT`/`WEBUI_ENABLED` 环境變數仍兼容，会自动转发到 API 服務。
...
- 检查啟動命令是否包含 `--serve` 或 `--serve-only` 參數
- 检查「访问」標籤页是否已配置域名
- 检查防火墙设置
...
确保它们共享同一个 `/app/data` 存储卷，以共享資料庫。

### 9.2 自定义域名

在 Zeabur 控制台的「访问」標籤页，你可以：
...
2. **配置合理的健康检查**：根据实际情况调整健康检查參數
3. **使用环境變數管理敏感資訊**：不要将 API 密钥硬编码到代碼中
4. **定期備份數據**：定期下載 `/app/data` 目錄的内容进行備份
5. **使用合适的啟動模式**：根据需求选择合适的啟動命令
6. **監控服務狀態**：定期检查服務狀態和日誌
