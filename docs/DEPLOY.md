...
| 方案 | 优点 | 缺点 | 推荐場景 |
|------|------|------|----------|
| **Docker Compose** ⭐ | 一键部署、環境隔離、易迁移、易升級 | 需要安裝 Docker | **推荐**：大多数場景 |
| **直接部署** | 簡單直接、无额外依賴 | 環境依賴、迁移麻烦 | 暫時測試 |
...
啟動成功后，在浏览器輸入 `http://服務器公网IP:8000` 即可打开 Web 管理界面。如果打不开，记得先在云服務器控制台的「安全组」里放行 8000 端口。

> 不知道怎么訪問？→ [云服務器 Web 界面訪問指南](deploy-webui-cloud.md)
...
如果你显式指定了 `--user` / Compose `user:`，或使用只读挂载、rootless Docker、NFS 等不允許容器修复属主的環境，请確保實際執行使用者对这些目錄具备写入權限。
...
# 啟動 Web 界面（啟動时執行一次分析；需每日定时请加 --schedule 或设 SCHEDULE_ENABLED=true）
...
| 配置项 | 说明 | 獲取方式 |
|--------|------|----------|
| `ANSPIRE_API_KEYS` / `AIHUBMIX_KEY` / `GEMINI_API_KEY` / `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` | AI 模型至少配置一个；推荐優先 Anspire 或 AIHubMix | 对应服務商控制台 |
...
| `SCHEDULE_TIME` | `18:00` | 每日執行时间 |
| `MARKET_REVIEW_ENABLED` | `true` | 是否启用大盤复盘 |
| `ANSPIRE_API_KEYS` | - | Anspire 大模型与新闻搜尋（推荐） |
| `AIHUBMIX_KEY` | - | AIHubMix 一 Key 多模型（推荐） |
| `SERPAPI_API_KEYS` | - | SerpAPI 实时金融新闻搜尋（推荐） |
| `TAVILY_API_KEYS` | - | Tavily 新闻搜尋（可選） |
...
### 5. WebUI 打开后 UI 元素例外变大 / 布局错乱

**症状**：能訪問 8000 端口，但页面上的文字、按钮、卡片例外放大，没有正常布局。

**根因**：`static/index.html` 存在，但 CSS/JS 资源文件缺失（`static/assets/` 为空或不存在），浏览器無法加载样式与脚本，导致裸 HTML 渲染。

**解決方法**：

- **Docker 部署**：執行以下命令重新构建镜像（確保前端已正确打包进镜像）：
  ```bash
  docker-compose -f ./docker/docker-compose.yml down
  docker-compose -f ./docker/docker-compose.yml build --no-cache
  docker-compose -f ./docker/docker-compose.yml up -d
  ```
  构建完成后刷新浏览器快取（`Ctrl+Shift+R`）再訪問。
...
**驗證**：用浏览器开发者工具（F12 → Network）檢查是否有 `/assets/index-*.js` 和 `/assets/index-*.css` 的 404 錯誤；如有，说明资源缺失，按上述步骤重新构建即可。
...
| Secret 名称 | 说明 | 必填 |
|------------|------|------|
| `ANSPIRE_API_KEYS` | Anspire Open API Key（一 Key 启用大模型与搜尋） | 推荐 |
...
| `GEMINI_MODEL` | 模型名称（預設 gemini-2.0-flash） | 可選 |
...
## 🌐 云服務器上部署了，但不知道怎么用浏览器訪問？

详见 → [云服務器 Web 界面訪問指南](deploy-webui-cloud.md)

涵盖：直接部署和 Docker 两种方式的啟動与訪問、安全组/防火墙配置、常见議題排查、Nginx 反向代理（可選）。
