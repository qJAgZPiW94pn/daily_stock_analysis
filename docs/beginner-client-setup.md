...
这份文档写给不会代碼、只想下載客户端直接用的使用者。目标很簡單：下載客户端，填一个模型服務密钥（Key），填股票代碼，然后生成第一份分析报告。

> 本项目生成的是辅助分析报告，不构成投资建议。真实交易请自行判斷風險。

## 先准备

1. Windows 或 macOS 电脑。
2. 一个模型服務密钥（Key），推荐从下面任选一个：
   - [Anspire Open](https://open.anspire.cn/?share_code=QFBC0FYC)：支援全球主流模型，一个 Key 可同时用于模型和新闻搜尋，第一次配置最省事。
   - [AIHubMix](https://aihubmix.com/?aff=CfMq)：支援全球主流模型，适合想在一个平台切換多种模型的使用者。
...
- Windows 安裝包：双击 `.exe`，按提示安裝，安裝目錄用預設位置即可。
- Windows 免安裝包：解壓 `.zip`，双击 `Daily Stock Analysis.exe`。
- macOS：双击 `.dmg`，把应用拖到 `Applications`。如果提示来自未驗證开发者，在系統设置的隐私与安全性里允許打开。
...
> 重要：每次改完设置后，都要点击页面上的保存按钮；看到保存成功提示后，再切換页面或回到首页。
