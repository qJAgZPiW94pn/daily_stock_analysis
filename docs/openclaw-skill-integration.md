...
- **适用场景**：已部署 DSA API 服務，希望在 openclaw 对话中触发分析（如「帮我分析茅台」「analyze AAPL」）

## 前置条件

1. **daily_stock_analysis 必须已執行**：执行 `python main.py --serve-only` 或通过 Docker 部署，使 API 长期可用
...
- **同步模式耗时**：`async_mode: false` 时，单次分析约 2–5 分钟，需确保 openclaw 或 HTTP 客户端逾時足够
...
description: 调用 daily_stock_analysis API 进行股票智能分析。当使用者询问「分析茅台」「analyze AAPL」「帮我看看 600519」等时使用。仅支援股票代碼，不支援中文名称。
metadata:
...
2. **调用 API**：向 `{DSA_BASE_URL}/api/v1/analysis/analyze` 发送 POST 請求，請求体：
   ```json
   {"stock_code": "<提取的代碼>", "report_type": "detailed", "force_refresh": true, "async_mode": false, "skills": ["bull_trend"]}
   ```
   > `skills` 为可選策略 ID 数组；历史欄位 `strategies` 仍保留兼容，建议优先使用 `skills`。
3. **等待回應**：同步模式下分析约需 2–5 分钟，请确保 HTTP 客户端逾時足够（建议 ≥300 秒）。
...
5. **錯誤處理**：
   - 連線失败：提示检查 DSA 是否執行、DSA_BASE_URL 是否正确
   - 400：检查 stock_code 格式
   - 409：该股票正在分析中，可稍后重试或查詢工作狀態
   - 500：提示查看 DSA 日誌排查
...
若 daily_stock_analysis 已啟用 `AGENT_MODE=true`，可调用 Agent 策略问股接口，支援多轮对话与多种策略（缠论、均线金叉等）：
...
| 现象 | 可能原因 | 處理建议 |
|------|----------|----------|
| 連線失败 | DSA 未執行、端口錯誤、防火墙 | 确认 `python main.py --serve-only` 已啟動，检查 `DSA_BASE_URL` |
| 400 錯誤 | stock_code 格式錯誤或缺失 | 检查代碼格式（见上文表格），确保請求体包含 `stock_code` |
...
默认情况下 DSA API 無需認證。若在 `.env` 中启用了 `ADMIN_AUTH_ENABLED=true`，则需在 Skill 调用时携带登录后获得的 Cookie，具体方式取决于 openclaw 的 HTTP 工具能力（当前 API 仅支援 Cookie 認證，不支援 Bearer Token）。
