...
本文档整理了使用者在使用过程中遇到的常见議題及解決方案。

---

## 📊 數據相關

### Q1: 美股代碼（如 AMD, AAPL）分析时价格顯示不正确？

**現象**：輸入美股代碼后，顯示的价格明显不对（如 AMD 顯示 7.33 元），或被误识别为 A 股。

**原因**：早期版本代碼匹配邏輯優先嘗試国内 A 股規則，导致代碼冲突。
...
2. 如仍有議題，可在 `.env` 中设置：
   ...
   YFINANCE_PRIORITY=0
   ```
   这将優先使用 Yahoo Finance 數據源獲取美股數據

> 📌 相關 Issue: [#153](https://github.com/ZhuLinsen/daily_stock_analysis/issues/153)

---

### Q2: 报告中"量比"欄位顯示为空或 N/A？

**現象**：分析报告中量比數據缺失，影响 AI 对缩放量的判斷。

**原因**：預設的某些实时行情源（如新浪介面）不提供量比欄位。

**解決方案**：
1. 已在 v2.3.0 修复，腾讯介面现已支援量比解析
2. 推荐配置实时行情源優先级：
   ```bash
   REALTIME_SOURCE_PRIORITY=tencent,akshare_sina,efinance,akshare_em
   ```
3. 系統已内置 5 日均量计算作为兜底邏輯

> 📌 相關 Issue: [#155](https://github.com/ZhuLinsen/daily_stock_analysis/issues/155)

---

### Q3: Tushare 獲取數據失败，提示 Token 不对？

**現象**：日誌顯示 `Tushare 獲取數據失败: 您的token不对，請確認`
...
**現象**：日誌顯示 `熔斷器触发` 或數據傳回 `None`，或出现 `RemoteDisconnected`、`push2his.eastmoney.com` 連線被關閉等

**原因**：免费數據源（东方财富、新浪等）有反爬機制，短时间大量請求会被限流。

**解決方案**：
1. 系統已内置多數據源自动切換和熔斷保护
2. 减少自选股数量，或增加請求间隔
3. 避免頻繁手动触发分析
4. 若东财介面頻繁失败，可设置 `ENABLE_EASTMONEY_PATCH=true` 启用东财补丁（注入 NID 令牌与隨機 User-Agent，降低被限流機率）
5. 将 `MAX_WORKERS=1` 改为串行獲取，减少对东财的並行压力

---

## ⚙️ 配置相關

### Q5: GitHub Actions 執行失败，提示找不到環境變數？

**現象**：Actions 日誌顯示 `GEMINI_API_KEY` 或 `STOCK_LIST` 未定义

**原因**：GitHub 区分 `Secrets`（加密）和 `Variables`（普通變數），配置位置不对会导致讀取失败。
...
2. **Docker 部署 / WebUI 系統设置**：
   ...
   - WebUI 保存后会触发当前程式的配置重載；執行中的讀取路徑会同步使用最新写回的 `.env`，例如定时工作会繼續热讀取保存后的 `STOCK_LIST`
   - 如果容器啟動命令里显式传入了这些同名環境變數（如 `docker run -e ...` 或 Compose `environment:`），后续重啟时仍以显式程式環境變數为准；要让 WebUI 保存值接管，请同步更新或移除这些显式 override
   - 其中 `SCHEDULE_*` 与 `RUN_IMMEDIATELY` 属于**啟動期调度配置**，保存后不会立即触发一次分析，也不会热重建当前程式里的 scheduler
   - 如需让调度开关立刻接管当前容器，请重啟容器，并確保以 schedule 模式啟動
3. **Docker 手工改 `.env` 后**：修改后仍建议重啟容器
   ```bash
   docker-compose down && docker-compose up -d
   ```
4. **GitHub Actions**：`.env` 文件不生效，必须在 Secrets/Variables 中配置
5. 檢查是否有多个 `.env` 文件（如 `.env.local`）导致覆盖
...
> ⚠️ 注意：代理配置仅对本機執行生效，GitHub Actions 環境無需配置代理。
...
系統按優先级只取一种：進階模型路由 YAML（`LITELLM_CONFIG`）> `LLM_CHANNELS` > legacy keys。但 YAML 仅在文件可正常解析且产出了有效 `model_list` 时才生效；如果 YAML 路徑无效或内容为空，系統会自动回退到 `LLM_CHANNELS` 或 legacy keys。一旦某一層級實際生效，更低優先级的配置不参与解析。

**Q: check_env 輸出“未配置可用 AI 模型”怎么办？**

預設先选一种服務商并填写对应 API Key；如果需要固定主模型，再补 `LITELLM_MODEL=provider/model`；如果要多模型切換，再配置 `LLM_CHANNELS` 或進階模型路由 YAML。執行 `python scripts/check_env.py --config` 校验配置，`python scripts/check_env.py --llm` 實際呼叫 API 測試。
...
先确认当前是否启用了 `LITELLM_CONFIG` 或 `LLM_CHANNELS`；如果启用了，上层配置会覆盖 legacy keys。若你没有启用这两层，且 `AGENT_LITELLM_MODEL` 为空，问股 Agent 仍会自动继承 legacy provider 模型：`GEMINI_MODEL`、`OPENAI_MODEL`、`ANTHROPIC_MODEL` 分别映射到对应 provider 前綴的 LiteLLM 模型名。此次修复不会静默迁移或清空旧配置，只是把“真实缺失原因”直接傳回到前端，便于你判斷到底是缺 key、缺模型名，还是被上层配置覆盖。完整相容语义见 [LLM 配置指南](LLM_CONFIG_GUIDE.md) 中“问股 Agent / LiteLLM 配置相容说明”。
...
3. 確保 Bot 已被添加到目标群組（如果是群聊）
4. 本機執行时需要能訪問 Telegram API（可能需要代理）

---

### Q10: 企业微信 Markdown 格式顯示不正常？

**解決方案**：
1. 企业微信对 Markdown 支援有限，可嘗試设置：
   ```bash
...
3. **模型名称是否加了 `ollama/` 前綴**
   - ✅ 正确：`LITELLM_MODEL=ollama/qwen3:8b`
   - ❌ 錯誤：`LITELLM_MODEL=qwen3:8b`（缺少前綴，litellm 無法路由到 Ollama）
...
5. **遠端部署 / Docker 时的網路与防火墙**
   - 若 Ollama 和程式不在同一主机，需将 `OLLAMA_API_BASE` 改为實際 IP，如 `http://192.168.1.100:11434`。
   - 确认防火墙已放行 11434 端口，且 Ollama 啟動时綁定了正确地址（`OLLAMA_HOST=0.0.0.0:11434`）。
...
### Q14.1: Docker 中網路/DNS 解析失败（如 api.tushare.pro、searchapi.eastmoney.com 無法解析）？

**現象**：日誌顯示 `Temporary failure in name resolution` 或 `NameResolutionError`，股票數據 API 和大模型 API 均無法訪問。

**原因**：自定义 bridge 網路下，容器使用 Docker 内置 DNS，在旁路由、特定網路環境时可能解析失败。
...
1. **显式配置 DNS**：在 `docker/docker-compose.yml` 的 `x-common` 下添加：
   ...
   dns:
     ...
     - 8.8.8.8
   ```
   然后執行 `docker-compose down` 和 `docker-compose up -d --force-recreate` 重新建立容器。

2. **改用 host 網路模式**：若上述仍无效，可在 `server` 服務下添加 `network_mode: host`，并移除 `ports` 映射。使用 host 模式时，`ports` 无效，**端口由 `command` 中的 `--port` 指定**。若宿主机預設端口已占用，可修改为其他端口（如 `.env` 中设置 `API_PORT=8080`），訪問对应 `http://localhost:8080`。
...
**结论**：对 Docker 使用者来说，**最权威的版本不是某个 Python 源文件常量，而是你實際使用的镜像 tag**。
...
**建议**：如果你想避免重复更新，部署时尽量固定使用明確的版本 tag（如 `v3.12.0`），不要长期依賴 `latest`。
...
### Q16: 分析结果中買入/观望/賣出数量統計不对？

**原因**：早期版本使用正则匹配統計，可能与實際建议不一致。

**解決方案**：已在最新版本中修复，AI 模型现在会直接輸出 `decision_type` 欄位用于准确統計。

---

### Q17: 为什么週末在 GitHub Actions 手动触发仍顯示“非交易日略過”？

**現象**：已經配置了 `TRADING_DAY_CHECK_ENABLED` 或希望手动執行，但日誌仍提示“今日所有相關市场均为非交易日，略過執行”。

**解決方案**：
1. 打开 `Actions → 每日股票分析 → Run workflow`
2. 手动触发时将 `force_run` 设为 `true`（单次強制執行）
3. 如果希望长期關閉交易日檢查，在 `Settings → Secrets and variables → Actions` 中设置：
   ```bash
   TRADING_DAY_CHECK_ENABLED=false
   ```

**規則说明**：
- `TRADING_DAY_CHECK_ENABLED=true` 且 `force_run=false`：非交易日略過（預設）
- `force_run=true`：本次即使非交易日也執行
- `TRADING_DAY_CHECK_ENABLED=false`：定时和手动都不做交易日檢查
