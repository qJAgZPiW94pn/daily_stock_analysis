...
class TestStorage(unittest.TestCase):
    ...
    def test_parse_sniper_value(self):
        ...
        # 2. 包含中文描述和"元"
        self.assertEqual(DatabaseManager._parse_sniper_value("建议在 100 元附近買入"), 100.0)
        self.assertEqual(DatabaseManager._parse_sniper_value("价格：100.5元"), 100.5)
        
        # 3. 包含干扰数字（修复的Bug場景）
        # 之前 "MA5" 会被錯誤提取为 5.0，现在应该提取 "元" 前面的 100
        text_bug = "無法给出。需等待MA5數據恢復，在股價回踩MA5且乖离率<2%时考虑100元"
        self.assertEqual(DatabaseManager._parse_sniper_value(text_bug), 100.0)
        
        # 4. 更多干扰場景
        text_complex = "MA10为20.5，建议在30元買入"
        self.assertEqual(DatabaseManager._parse_sniper_value(text_complex), 30.0)
        
        text_multiple = "支撑位10元，阻力位20元" # 应该提取最后一个"元"前面的数字，即20，或者更複雜的邏輯？
        # 当前邏輯是找最后一个冒号，然后找之后的第一个"元"，提取中间的数字。
        # 測試没有冒号的情况
